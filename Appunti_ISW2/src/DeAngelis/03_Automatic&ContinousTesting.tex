...
Un progetto Maven è organizzato secondo una struttura di directory standard, ad esempio:

\begin{itemize}
    \item \texttt{src/main/java};
    \item \texttt{src/test/java};
    \item \texttt{target}.
\end{itemize}

La directory \texttt{target} contiene tipicamente i risultati della build.

Il file principale di configurazione è il \texttt{pom.xml}, che contiene informazioni come:

\begin{itemize}
    \item \texttt{artifactId};
    \item \texttt{groupId};
    \item nome e versione del progetto;
    \item sviluppatori;
    \item dipendenze;
    \item direttive di build, test e deploy.
\end{itemize}
...
\begin{itemize}
    \item \textbf{JUnit 4}: riferimento del corso, basato su annotazioni;
    \item \textbf{JUnit 5}: evoluzione più modulare, con separazione tra API e SPI e maggiore flessibilità nella gestione dei runner.
\end{itemize}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.70\textwidth]{img/DeAngelis/03_Automatic&ContinuosTesting/JUNIT4}
...
\begin{notaesame}
Nel progetto bisogna configurare un ecosistema in cui \textbf{Git} traccia il codice, \textbf{Maven} gestisce build e dipendenze, e la \textbf{CI} esegue automaticamente build e test a ogni Pull Request.
\end{notaesame}

\begin{notaesame}
Se i file di test JUnit non rispettano i pattern riconosciuti da Maven, come \texttt{*Test.java}, Maven può ignorarli durante la fase di test.
\end{notaesame}

\begin{notaesame}
Il software testing trova failure esibite dal programma, ma è solo una parte della SQA. L'automazione serve ad abbattere i costi e a rendere ripetibili i test di regressione a ogni modifica del codice.
\end{notaesame}