\chapter{Unit and Integration Testing: Frameworks}

\section{Test di integrazione}

Il \textbf{test di integrazione} ha l'obiettivo di far emergere malfunzionamenti derivanti dall'interazione tra due o più moduli o componenti del \textbf{System Under Test} (SUT).
...
\begin{itemize}
    ...
    \item i moduli comunichino correttamente quando vengono composti.
\end{itemize}
...
Uno \textbf{stub} o \textbf{mock} simula il comportamento di un'unità mancante, non disponibile o non considerata nel test. Spesso tale comportamento è semplice, costante o controllato artificialmente.
...
\begin{itemize}
    ...
    \item simula una dipendenza richiesta dal SUT;
...
\begin{itemize}
    ...
    \item inizializzazione delle risorse;
    \item coordinamento tra componenti;
    \item clean-up dopo l'esecuzione;
    \item teardown dell'ambiente.
\end{itemize}
...
\begin{itemize}
    \item partendo dai moduli di livello superiore;
    \item simulando i moduli inferiori non ancora disponibili tramite stub;
...
In questo caso il raffinamento avviene considerando progressivamente interfacce più generali o parti più periferiche del sistema. I moduli superiori non ancora disponibili vengono simulati tramite \textbf{test driver}.
...
\textbf{Maven} è un sistema di build e gestione delle dipendenze sviluppato da Apache. È molto usato nei progetti Java ed è basato su un'architettura modulare a plug-in.
...
\begin{itemize}
    ...
    \item test;
    \item packaging;
    \item installazione e deploy;
    \item plug-in collegati al ciclo di build.
\end{itemize}

\begin{notaprof}
Maven gestisce automaticamente le dipendenze in modo transitivo. Lo sviluppatore dichiara le librerie nel \texttt{pom.xml}; Maven scarica da Maven Central sia le librerie indicate sia le loro sotto-dipendenze, mettendole in cache nella directory locale \texttt{.m2}.
\end{notaprof}

Il file \texttt{pom.xml} descrive il progetto e contiene:

\begin{itemize}
    \item coordinate del progetto;
    \item dipendenze;
    \item plug-in;
    \item configurazioni di build;
    \item informazioni sui moduli.
\end{itemize}
...
\begin{notaesame}
Le fasi Maven sono sequenziali. Un errore in una fase, per esempio un test fallito durante \texttt{test}, può interrompere la build e impedire l'esecuzione delle fasi successive.
\end{notaesame}
...
\begin{itemize}
    ...
    \item non agganciare i test automatici alla CI.
\end{itemize}