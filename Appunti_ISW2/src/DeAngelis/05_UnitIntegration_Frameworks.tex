...
\textbf{Maven} è un sistema di automazione della build, particolarmente usato in ambiente Java. Gestisce le dipendenze in modo esplicito e transitivo tramite repository come \textbf{Maven Central}.

Il file principale di configurazione è il \texttt{pom.xml}, in cui si dichiarano:
...
\begin{itemize}
    ...
    \item dipendenze;
    \item plugin;
    \item configurazioni di build e test.
\end{itemize}

\begin{notaslide}
Maven è un framework modulare basato su plugin. Ogni plugin aggiunge funzionalità specifiche e si innesta in una o più fasi del ciclo di build.
\end{notaslide}
...
\begin{notaslide}
Le fasi Maven sono organizzate in sequenza. Un errore in una fase comporta tipicamente il fallimento dell'intero processo di build.
\end{notaslide}
...
\begin{notaesame}
Nel \textbf{top-down} si usano soprattutto stub per simulare componenti di livello più basso non ancora disponibili. Nel \textbf{bottom-up} si usano soprattutto test driver per simulare i moduli chiamanti o più esterni.
\end{notaesame}

\begin{notaesame}
\textbf{Surefire} è associato ai test di unità e alla fase \texttt{test}; \textbf{Failsafe} è associato ai test di integrazione e alle fasi \texttt{integration-test} e \texttt{verify}.
\end{notaesame}

\begin{notaesame}
Un \textbf{mock} finge il comportamento di un oggetto configurandone output e interazioni. Una \textbf{spy} usa l'oggetto reale, ma permette di osservarne e verificarne le chiamate.
\end{notaesame}

\begin{notaesame}
Mockito serve a isolare il SUT e a controllare le interazioni, ma non sostituisce la progettazione del test: servono input significativi, oracoli corretti, assert e verifiche coerenti.
\end{notaesame}