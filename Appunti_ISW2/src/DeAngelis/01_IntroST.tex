...
\section{Introduzione al modulo}

Questa è la lezione introduttiva al modulo di \textbf{Software Testing} di \textbf{Ingegneria del Software 2}. Il modulo ha l'obiettivo di mostrare come migliorare la qualità del software in produzione attraverso processi, metodologie e tecnologie di testing.

Il corso è tenuto da \textbf{Guglielmo De Angelis}, ricercatore presso il gruppo \textbf{SaKS} del \textbf{CNR (IASI)} di Roma. Le sue aree di ricerca includono:
\begin{itemize}
    \item modellazione dei sistemi software;
    \item modellazione di proprietà extra-funzionali;
    \item generazione automatica a partire da modelli;
    \item architetture orientate ai servizi;
    \item software testing.
\end{itemize}

\section{Informazioni organizzative}

Le lezioni si tengono il \textbf{mercoledì}, dalle \textbf{11:30 alle 13:30}, in \textbf{Aula B14}, fino al \textbf{3 giugno 2026}.

\begin{notaprof}
Di comune accordo con l'aula, si faranno due ore di lezione consecutive, iniziando in orario e finendo puntuali, senza pause intermedie. Non sono previsti ponti o riduzioni del calendario.
\end{notaprof}

\subsection{Canali Teams}

Sono previsti due canali principali:
\begin{itemize}
    \item il \textbf{canale Generale}, usato per informazioni di carattere generale e per la condivisione delle slide;
    \item il \textbf{canale Software Testing}, dedicato alle domande specifiche sugli argomenti del modulo e sul progetto d'esame.
\end{itemize}

\subsection{Ricevimento e comunicazione}

\begin{notaprof}
È preferibile porre i propri dubbi direttamente in aula, così che tutti possano ascoltare le risposte. In alternativa, è fortemente consigliato usare la mailing list associata al canale Teams, in modo da discutere i problemi collettivamente anche nella lezione successiva.
\end{notaprof}

\begin{notaesame}
Il docente non legge in modo sincrono i messaggi diretti su Teams. Le richieste vanno fatte sempre via mail, preferibilmente mettendo in copia il canale Teams del modulo. Le mail private al docente devono essere usate solo per questioni strettamente personali.
\end{notaesame}

Quando il corso non è attivo, il ricevimento avviene solo su appuntamento via email.

\section{Sillabo del modulo}

Il sillabo segue una progressione che parte dalle basi e arriva fino alla generazione automatica dei test:
\begin{itemize}
    \item \textbf{Intro and General Concepts};
    \item \textbf{Test Automation e Continuous Testing};
    \item \textbf{Unit and Integration Testing};
    \item \textbf{Approaches to Test Generation};
    \item \textbf{Adequacy of the Tests: Control Flow Coverage};
    \item \textbf{Adequacy of the Tests: Mutation Testing};
    \item \textbf{Coverage-based Approaches to Test Generation}.
\end{itemize}

\section{Materiale didattico}

\begin{notaslide}
Tra i testi di riferimento compaiono, tra gli altri, \textbf{A.P. Mathur} con \emph{Foundations of software testing}, \textbf{B. Garcia} e \textbf{P. Tahchiev et al.} per la parte su \textbf{JUnit}, \textbf{W.E. Lewis} per il testing e il miglioramento continuo della qualità, e \textbf{J. Humble} con \textbf{D. Farley} per la \emph{Continuous Delivery}.
\end{notaslide}

\begin{notaprof}
Le slide sono sintesi drastiche dei concetti. Studiare solo sulle slide non basta: è necessario approfondire sui libri. Il testo di Mathur è molto utile per la parte teorica, mentre i libri su JUnit sono più vicini a una manualistica tecnica da consultare per la pratica.
\end{notaprof}
