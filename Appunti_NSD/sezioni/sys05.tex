...
\caption{Componenti fondamentali del controllo degli accessi: il soggetto che effettua la richiesta, l’azione richiesta e l’oggetto su cui l’operazione deve essere applicata.}
\label{fig:accessi}
\end{figure}
...
\texttt{CAP\_SYS\_ADMIN} & Copre molte operazioni amministrative. \\
\bottomrule
\end{tabular}
\end{center}
...
\caption{Modello del Reference Monitor: tutte le system call dei processi passano attraverso gli hook del sistema operativo, che inoltrano le richieste al Reference Monitor. Il modulo di autorizzazione consulta il Policy Store (stati di protezione, labeling e transizione) per decidere se consentire o negare ogni operazione.}
...
\begin{figure}[H]
    ...
    \caption{Esempio di Domain ACL in Tomoyo Linux: la sequenza di eseguibili attraversati definisce il dominio di sicurezza del processo, e le ACL specificano i permessi concessi per ciascun dominio lungo il percorso di esecuzione.}
    \label{fig:domainacl}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Sequenza semplificata del processo di boot: dal bootloader al kernel, all’initramfs e infine ai servizi di sistema. Sono evidenziati i punti in cui vengono applicate firme digitali e controlli MAC, insieme al punto debole rappresentato dall’attacco “Horse Pills” sul ramdisk.}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=.3\linewidth]{immagini/SYS_05/horse}
