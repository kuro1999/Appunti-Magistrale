...
\subsubsection{Perché la LAN Ethernet è fragile per natura}
...
\begin{figure}[H]
    ...
    \caption{Esempio di apprendimento degli indirizzi MAC in uno switch:
(i) quando arriva un frame con indirizzo sorgente non presente nel forwarding database viene creata una nuova entry e l’ageing viene impostato a~0;
(ii) quando il MAC sorgente è già presente nella tabella sulla stessa porta, viene semplicemente aggiornato l’ageing time.}
    \label{fig:lab_1}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Aggiornamento del forwarding database in caso di MAC sorgente già presente ma associato a una porta diversa: lo switch aggiorna la porta di ingresso e resetta l’ageing time, riflettendo la nuova posizione della stazione nella topologia.}
    \label{fig:lab_2}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Sequenza dei messaggi DHCP: il client invia un \texttt{DHCPDISCOVER} in broadcast, il server risponde con un \texttt{DHCPOFFER}, il client conferma con \texttt{DHCPREQUEST} e il server conclude con \texttt{DHCPACK}, assegnando l’indirizzo IP.}
...
\begin{figure}[H]
    ...
    \caption{Scambio ARP tra due host: l’host A invia un \texttt{ARP Request} in broadcast per conoscere il MAC associato all’indirizzo IP di destinazione, mentre l’host B risponde con un \texttt{ARP Reply} unicast fornendo il proprio MAC address.}
...
\begin{figure}[H]
    ...
    \caption{Attacco alla topologia STP: un host malevolo invia BPDU con priorità più bassa per farsi eleggere come root bridge, alterando la convergenza dello spanning tree e intercettando il traffico di rete.}
...
\newpage
%\maketitle % una seconda \maketitle non è necessaria
\section*{Laboratorio}
