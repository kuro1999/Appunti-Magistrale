...
\includegraphics[width=.5\linewidth]{immagini/SYS_04/strcmp.png}
    \caption{esempio di utilizzo di funzione strcmp}
\end{figure}
...
\includegraphics[width=.85\linewidth]{immagini/SYS_04/constTime.png}
    \caption{esempio di funzione constantTimeStringCompare}
\end{figure}
...
\caption{Schema operativo dell’attacco Meltdown: dopo il flush della cache, un accesso speculativo a memoria kernel carica transientemente un valore segreto, che viene usato come indice per accedere a \texttt{probe\_array}. Le linee cache così selettivamente caricate vengono poi identificate tramite misure di tempo (\textit{Flush+Reload}), rivelando il byte illegittimamente letto.}
...
\caption{Illustrazione del meccanismo di branch prediction sfruttato da Spectre: l’attaccante allena il predittore inducendo la CPU a scegliere sistematicamente il ramo “true”, anche quando la condizione \texttt{<in bounds>} sarebbe falsa. L’esecuzione speculativa prosegue nel ramo errato lasciando effetti micro-architetturali osservabili tramite side-channel.}
\end{figure}
...
\caption{Attacco Spectre basato su salti indiretti: l’attaccante avvelena il Branch Target Buffer (BTB) inducendo il predittore a deviare il salto verso uno \textit{spectre gadget} presente nello spazio di esecuzione della vittima. Il gadget viene eseguito speculativamente e accede a memoria in modo tale da codificare il dato nella cache, pur essendo il codice della vittima logicamente corretto.}
...
\caption{Meccanismo del L1 Terminal Fault (L1TF): durante la page-table walk, un indirizzo non valido produce un fault architetturale, ma la CPU speculativamente utilizza comunque l’indirizzo fisico transitorio presente in L1D. Questo consente all’attaccante di bypassare le verifiche di validità delle pagine (PT, EPT, SGX) e accedere speculativamente a dati sensibili prima che il fault venga gestito.}
\end{figure}
...
\begin{enumerate}
  ...
  \item \textbf{Encode/Leak}. Usare un gadget di encodifica basato su cache (stile Meltdown) e misurare il canale per ricostruire il contenuto protetto.
\end{enumerate}
Nel caso \textbf{VMM}, manipolando le \textbf{PT} del guest si può arrivare a \emph{osservare} memoria dell’host o di altri guest co–locati, sfruttando la finestra transiente e la condivisione delle strutture di traduzione.
\begin{figure}[H]
\centering
\includegraphics[width=.5\linewidth]{immagini/SYS_04/vm.png}
\caption{Scenario di attacco in ambiente cloud con Hyper-Threading: due VM appartenenti a tenant diversi condividono lo stesso core fisico ma su thread logici distinti. L’attaccante sfrutta la condivisione delle cache di livello L1/L2 per osservare effetti micro-architetturali lasciati dalla vittima e recuperare informazioni sensibili tramite side-channel.}
...
\caption{Esempio di monitoraggio tramite performance counters: l’analisi temporale dei contatori hardware evidenzia pattern anomali associati all’esecuzione speculativa o a side-channel micro-architetturali, utili per rilevare attività sospette o attivare mitigazioni selettive.}
...
\caption{Effetto della KPTI (Kernel Page-Table Isolation): quando la CPU è in user mode, le pagine del kernel vengono completamente rimosse dallo spazio di indirizzamento per impedire attacchi come Meltdown; al passaggio in kernel mode, la mappatura completa del kernel viene ripristinata.}
...
\end{itemize}
Queste tecniche contribuiscono a ridurre il rischio di attacchi come Spectre, migliorando la sicurezza dei processori moderni contro gli attacchi di predizione dei salti indiretti.
...
\caption{Struttura semplificata di un banco DRAM: la selezione della riga avviene tramite il segnale RAS, mentre la selezione della colonna è controllata dal segnale CAS. I dati vengono letti o scritti nella cella indirizzata attraverso le linee di bit, con il segnale R/W che determina la direzione dell’operazione.}
\end{figure}
