...
gli effetti micro–architetturali (es.\ cache) restano osservabili via side–channel. :contentReference[oaicite:0]{index=0}
...
Il miss–training efficace richiede in genere \emph{lo stesso core} della vittima; la fase di misurazione del canale
(\emph{timing}/cache) può avvenire anche su altri core. :contentReference[oaicite:2]{index=2}
...
\end{verbatim}
Anche se la condizione è falsificata, durante la finestra speculativa la CPU può accedere a \texttt{array1[x]} fuori limite
e toccare \texttt{array2[ ... ]}, lasciando un’impronta in cache che rivela \texttt{array1[x]}.

\paragraph{Fasi sintetiche}
\begin{enumerate}
  \item \textbf{Miss-train} del ramo per rendere probabile la predizione ``true'' su \texttt{x < array1\_size}.
  \item \textbf{Trigger} con \texttt{x} fuori limite: accesso speculativo a \texttt{array1[x]}.
  \item \textbf{Encode} nel canale: accesso a \texttt{array2[array1[x]*4096]}.
  \item \textbf{Leak} via \emph{Flush+Reload}/\emph{Prime+Probe} su \texttt{array2} per ricostruire il byte.
\end{enumerate}
:contentReference[oaicite:4]{index=4}

\paragraph{Note dalle slide}
Il vendor-specific \emph{bypass} consente alla speculazione di attraversare il check; l’osservazione dello stato di cache
di \texttt{array2} permette di inferire il valore letto speculativamente. :contentReference[oaicite:5]{index=5}
...
(anche in eBPF o in librerie condivise), poi \textbf{avvelena il BTB} per far \emph{mispredict} il salto indiretto verso quel gadget,
che viene eseguito speculativamente. Il codice della vittima non è ``buggato'': è sfruttato come gadget per eseguire
accessi che \emph{encodano} il dato in cache. Questo è concettualmente vicino alla ROP. :contentReference[oaicite:6]{index=6}
...
Esempi e note di impatto (CVE-2017-5753, CVE-2017-5715) sono riportati nelle slide. :contentReference[oaicite:8]{index=8}

\paragraph{Contromisure (rimando)}
Tecniche di hardening come \emph{Retpoline} (thunks di ritorno che intrappolano la speculazione) e le estensioni ISA
\emph{IBRS/STIBP/IBPB} riducono l’efficacia del poisoning dei salti indiretti (si veda la sezione sulle contromisure).

\subsubsection{L1TF (Foreshadow): principio e flusso}
Durante la traduzione degli indirizzi, i processori Intel eseguono in \emph{parallelo} il flusso di traduzione (TLB, page-table walk, eventuale EPT walk per la VM, e verifica SGX) e la lookup nella \textbf{cache L1}, che è \emph{virtual-indexed, physically-tagged}. In questa finestra \emph{transient}, una voce di page table \emph{non valida} può essere \emph{speculativamente propagata} alla CPU: l’effetto risultante in L1 può essere osservato con un canale laterale “alla Meltdown” (le tracce rimangono in cache nonostante il flush architetturale). :contentReference[oaicite:0]{index=0}

\paragraph{Perché funziona}
L’accesso a una pagina che deve generare \emph{terminal fault} (voce PTE non presente o riservata) viene comunque speculato fino a lasciare side–effects in L1; poiché la lookup L1 procede mentre i controlli di privilegio/validità avanzano, l’indirizzo fisico speculativo può guidare una \emph{encode} nel canale (es.\ tocchi di una probe array), recuperabile poi via timing. L1TF, così, \textbf{bypassa} tre barriere: protezioni del SO (PT walk), dell’hypervisor (EPT walk) e SGX. :contentReference[oaicite:1]{index=1}

\paragraph{Varianti e impatto}
La famiglia include: \textbf{Foreshadow-SGX} (CVE-2018-3615), \textbf{Foreshadow-OS} (CVE-2018-3620), \textbf{Foreshadow-VMM} (CVE-2018-3646); scoperta a inizio 2018 e divulgata il 14 agosto 2018. Colpisce gran parte delle CPU Intel Core/Xeon (specie con SGX e virtualizzazione); AMD e ARM risultano non affette nello specifico L1TF.

\paragraph{Schema operativo (dalle slide)}
\begin{enumerate}
  \item \textbf{Preparazione PTE/EPT}: creare una voce di page table \emph{per un indirizzo d’interesse} e marcarla come \emph{invalida} (bit \textit{present}/\textit{reserved} a 0) per generare un \emph{terminal fault}. \textit{Nota:} il kernel in genere non lo consente, ma un attaccante con \emph{guest OS} su una VM può manipolare le proprie PT. :contentReference[oaicite:3]{index=3}
  \item \textbf{Trigger transiente}: causare l’accesso che, pur faultando, porta temporaneamente il dato in L1.
  \item \textbf{Encode/leak}: usare un gadget di encode (cache-based) e misurare il canale per ricostruire il contenuto protetto (stile Meltdown). :contentReference[oaicite:4]{index=4}
\end{enumerate}
Nello scenario VMM, manipolando le PT del guest è possibile \emph{osservare} memoria dell’host o di altri guest co-locati sfruttando la finestra transiente e la condivisione delle strutture di traduzione. :contentReference[oaicite:5]{index=5}

\paragraph{Mitigazioni (riassunto dalle slide \& appunti)}
\begin{itemize}
  \item \textbf{Hardening della microarchitettura/firmware \& OS/VMM}: aggiornamenti di microcode e patch che impediscono l’uso transiente di PTE non valide e mitigano l’esposizione durante i confini di privilegio/VM. (Vedi anche la separazione KPTI per Meltdown, concettualmente affine come scopo di ridurre superfici transiente utili.)
  \item \textbf{Mitigazioni specifiche per L1}: gli attacchi \emph{cross-process} su L1 richiedono condivisione della L1 tra thread \emph{(SMT/Hyper-Threading)}; \textbf{disabilitare SMT} elimina quella via di attacco.
  \item \textbf{Riduzione della superficie dei side channel}: limitare timer ad alta risoluzione, marcatura di regioni non cacheabili, implementazioni \emph{constant-time} e disabilitazione TSX dove rilevante per canali come Prime+Abort. (Costi prestazionali non trascurabili.)
  \item \textbf{Rilevazione}: l’allestimento del canale crea \emph{rumore} (miss/flush/abort) osservabile con \emph{Hardware Performance Counters}; utile per detection adattiva, con limiti di portabilità tra vendor/generazioni.
\end{itemize}
