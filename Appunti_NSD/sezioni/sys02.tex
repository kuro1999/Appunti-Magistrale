...
\begin{figure}[H]
    ...
    \caption{Pattern History Table (PHT) utilizzata da un predittore correlato (two-level): il contenuto del Global History Register (sequenza degli ultimi salti presi/non presi) viene usato come indice per selezionare il contatore di predizione associato a quello specifico pattern di branch history.}
    \label{fig:pht}
\end{figure}
...
\begin{itemize}
  ...
  \begin{figure}[H]
      ...
      \caption{Modello di sistema cache con protocollo di \textit{snooping}: ogni core dispone di una cache privata con relativo controller, mentre la coerenza viene mantenuta monitorando (snoop) il traffico sull’interconnessione condivisa. La LLC e il memory controller completano la gerarchia comunicando con la memoria principale.}
      \label{fig: snooping}
  \end{figure}
